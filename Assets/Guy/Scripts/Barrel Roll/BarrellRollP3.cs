...
public class BarrelRollP3 : MonoBehaviour
{
    ...
    void Update()
    {
        if (!isRolling)
        {
            if (Input.GetButtonDown("LTJoystick3")) // Replace with your actual input name
            {
                StartCoroutine(BarrelRoll(-1));
            }
            else if (Input.GetButtonDown("RTJoystick3")) // Replace with your actual input name
            {
                StartCoroutine(BarrelRoll(1));
            }
        }
